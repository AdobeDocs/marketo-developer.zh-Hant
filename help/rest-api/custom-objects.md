...
或者，使用複合金鑰進行查詢時，API的行為類似於Opportunity Roles API，接受具有JSON主體的POST。  JSON內文可以有與GET查詢相同的成員，但`filterValues`除外。  有一個`input`陣列取代篩選值，它採用物件，其中包含每個物件型別`dedupeFields`的命名成員。
